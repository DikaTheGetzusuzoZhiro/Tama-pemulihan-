...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    const lookup=await axios.post("https://users.roblox.com/v1/usernames/users",
      {usernames:[username],excludeBannedUsers:false},{timeout:9000});
    const user=lookup.data?.data?.[0];
    if(!user) return send(res,404,{error:"Username Roblox tidak ditemukan."});
    const avatar=await axios.get("https://thumbnails.roblox.com/v1/users/avatar-headshot",{
  ...
  }catch(e){return send(res,502,{error:"Gagal mengambil data Roblox. Coba lagi."});}
};